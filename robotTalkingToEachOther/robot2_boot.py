...
print("Starting robot2 controller...")
...
def set_angle(angle):
    ...
    servo_pin.duty(duty)
    print(f"Robot2 servo -> {angle}deg")

set_angle(0)  # Robot2 starts at 0 (opposite of robot1's 90)
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; touch-action: none; }
  ...
  h1 { color: #ff6b35; font-size: 1rem; letter-spacing: 0.3em; text-transform: uppercase; opacity: 0.8; }
  .servo-section {
    display: flex;
    flex-direction: column;
    align-items: center;
  ...
  .section-label { color: #ff6b35; font-size: 0.65rem; letter-spacing: 0.3em; opacity: 0.5; text-transform: uppercase; }
  .note { color: #ff6b3566; font-size: 0.6rem; letter-spacing: 0.15em; text-align: center; }
  .dial-row { display: flex; align-items: center; gap: 24px; }
  canvas { cursor: pointer; }
  .angle-display { color: #ff6b35; font-size: 2.4rem; letter-spacing: 0.1em; text-shadow: 0 0 20px #ff6b3588; min-width: 90px; text-align: center; }
  ...
  input[type=range] {
    ...
    background: linear-gradient(to right, #ff6b35 var(--pct), #1a1a1a var(--pct));
    outline: none; border: none;
  }
  input[type=range]::-webkit-slider-thumb {
    -webkit-appearance: none; width: 24px; height: 24px; border-radius: 50%;
    background: radial-gradient(circle at 35% 35%, #ff6b35, #cc4400);
    box-shadow: 0 0 12px #ff6b3577; cursor: grab;
  }
  .tick-row { display: flex; justify-content: space-between; width: 100%; color: #ff6b3555; font-size: 0.6rem; letter-spacing: 0.1em; }
  .presets { display: flex; gap: 10px; }
  .preset-btn {
    background: #111; border: 1px solid #ff6b3533; color: #ff6b35;
  ...
  .preset-btn:hover, .preset-btn:active { background: #ff6b3522; box-shadow: 0 0 12px #ff6b3533; }
  ...
  .joy-label { color: #ff6b35; font-size: 0.65rem; letter-spacing: 0.2em; opacity: 0.5; }
  .joystick-zone {
    width: 140px; height: 140px; border-radius: 50%;
    background: radial-gradient(circle, #1a1a1a, #0d0d0d);
    border: 2px solid #ff6b3533; position: relative;
    box-shadow: 0 0 30px #ff6b3522, inset 0 0 20px #00000066;
  }
  .joystick-knob {
    ...
    box-shadow: 0 0 20px #ff6b3566; transition: box-shadow 0.1s; cursor: grab;
  }
  .joystick-knob.active { box-shadow: 0 0 40px #ff6b35aa; }
  .status { color: #ff6b3555; font-size: 0.65rem; letter-spacing: 0.2em; }
</style>
</head>
<body>

<h1>&#9632; Robot2 Control</h1>

<div class="servo-section">
  <div class="section-label">Servo</div>
  <div class="note">&#8594; Mirrors Robot1 (inverted)</div>
  <div class="dial-row">
    <canvas id="dial" width="160" height="160"></canvas>
    <div class="angle-display"><span id="angleNum">0</span><span class="angle-unit">°</span></div>
  </div>
  <div class="slider-wrap">
    <input type="range" id="slider" min="0" max="180" value="0" style="--pct:0%">
    <div class="tick-row">
...
function drawDial(angle) {
  ...
  ctx.strokeStyle = '#ff6b35'; ctx.lineWidth = 9; ctx.lineCap = 'round'; ctx.stroke();
  [0, 45, 90, 135, 180].forEach(a => {
    ...
    ctx.strokeStyle = '#ff6b3544'; ctx.lineWidth = 2; ctx.stroke();
  });
...
stop()
print("Robot2 web server running...")
